...
class FruitActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //set title
        tvTitle.setText("Toko Buah Terdekat")
        ...
        listResultViewModel.setFruitLocation(strLokasi)
        progressDialog.show()
        listResultViewModel.getFruitLocation().observe(this, { modelResults: ArrayList<ModelResults>? ->
            if (modelResults!!.size != 0) {
                mainAdapter.setResultAdapter(modelResults)
                progressDialog.dismiss()
            } else {
                Toast.makeText(this, "Oops, lokasi Toko Buah tidak ditemukan!", Toast.LENGTH_SHORT).show()
            }
        })
    }
