...
class SupermarketActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //set title
        tvTitle.setText("Supermarket Terdekat")
        ...
        listResultViewModel.setSupermarketLocation(strLokasi)
        progressDialog.show()
        listResultViewModel.getSupermarketLocation().observe(this, { modelResults: ArrayList<ModelResults>? ->
            if (modelResults!!.size != 0) {
                mainAdapter.setResultAdapter(modelResults)
                progressDialog.dismiss()
            } else {
                Toast.makeText(this, "Oops, lokasi Supermarket tidak ditemukan!", Toast.LENGTH_SHORT).show()
            }
        })
    }
