...
class VegetableActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //set title
        tvTitle.setText("Toko Sayur Terdekat")
        ...
        listResultViewModel.setVegetableLocation(strLokasi)
        progressDialog.show()
        listResultViewModel.getVegetableLocation().observe(this, { modelResults: ArrayList<ModelResults>? ->
            if (modelResults!!.size != 0) {
                mainAdapter.setResultAdapter(modelResults)
                progressDialog.dismiss()
            } else {
                Toast.makeText(this, "Oops, lokasi Toko Sayur tidak ditemukan!", Toast.LENGTH_SHORT).show()
            }
        })
    }
